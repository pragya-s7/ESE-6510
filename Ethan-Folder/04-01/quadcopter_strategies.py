...
class DefaultQuadcopterStrategy:
    ...
    def get_rewards(self) -> torch.Tensor:
        ...
        if len(valid_last_ids) > 0:
            ...
            ###### SAME-AXIS OVER-THE-TOP RULE START ######
            """current_target_idx = current_gate_idx[valid_last_ids]
            last_gate_pos = self.env._waypoints[last_gate_idx, :3]
            current_gate_pos = self.env._waypoints[current_target_idx, :3]
            pair_delta = current_gate_pos - last_gate_pos

            pair_dist_xy = torch.linalg.norm(pair_delta[:, :2], dim=1)
            same_exit_direction = torch.cos(
                wrap_to_pi(self.env._waypoints[current_target_idx, -1] - self.env._waypoints[last_gate_idx, -1])
            ) > 0.95
            same_x_axis = pair_delta[:, 0].abs() < 0.2
            same_y_axis = pair_delta[:, 1].abs() < 0.2
            share_axis = same_x_axis | same_y_axis
            close_enough = pair_dist_xy < 1.0
            similar_height = pair_delta[:, 2].abs() < 0.35
            special_pair = same_exit_direction & share_axis & close_enough & similar_height

            seg_xy = current_gate_pos[:, :2] - last_gate_pos[:, :2]
            seg_xy_norm_sq = torch.sum(seg_xy * seg_xy, dim=1).clamp_min(1e-6)
            drone_rel_xy = drone_pos_w[valid_last_ids, :2] - last_gate_pos[:, :2]
            t = torch.sum(drone_rel_xy * seg_xy, dim=1) / seg_xy_norm_sq
            closest_xy = last_gate_pos[:, :2] + t.unsqueeze(1) * seg_xy
            lateral_dist = torch.linalg.norm(drone_pos_w[valid_last_ids, :2] - closest_xy, dim=1)
            between_gates = (t > 0.1) & (t < 0.9)

            gate_top_height = torch.maximum(last_gate_pos[:, 2], current_gate_pos[:, 2])
            above_top_route = drone_pos_w[valid_last_ids, 2] > (gate_top_height + 0.25)
            went_around_outside = between_gates & (lateral_dist > 0.45) & (~above_top_route)
            cheated[valid_last_ids] |= special_pair & went_around_outside"""
            ###### SAME-AXIS OVER-THE-TOP RULE END ######
    ...
    def reset_idx(self, env_ids: Optional[torch.Tensor]):
        ...
        if self.cfg.is_train:
            ...
            gate0_mask = torch.rand(n_reset, device=self.device) < 0.3
