...
class DefaultQuadcopterStrategy:
    ...
    def get_rewards(self) -> torch.Tensor:
        ...
        if len(valid_last_ids) > 0:
            ...
            ### Powerloop ###
            # current_target_idx = current_gate_idx[valid_last_ids]
            # last_gate_pos = self.env._waypoints[last_gate_idx, :3]
            # curr_gate_pos = self.env._waypoints[current_target_idx, :3]
            # delta_p = curr_gate_pos - last_gate_pos
            #
            # last_gate_normal = self.env._normal_vectors[last_gate_idx]
            # curr_gate_normal = self.env._normal_vectors[current_target_idx]
            # same_normal = torch.sum(last_gate_normal * curr_gate_normal, dim=1) > 0.98
            #
            # same_entry_plane = delta_p[:, 0].abs() < 0.2
            # close_enough = torch.linalg.norm(delta_p, dim=1) < 1.0
            # special_pair = same_normal & same_entry_plane & close_enough
            #
            # gate_top_height = torch.maximum(last_gate_pos[:, 2], curr_gate_pos[:, 2] + gate_half_extent)
            # over_top = drone_pos_w[valid_last_ids, 2] > (gate_top_height + 0.25)
            #
            # self.env._special_pair_over_top_satisfied[valid_last_ids] |= special_pair & over_top
            #
            # cheated_special_pair_pass[valid_last_ids] = (
            #     gate_passed[valid_last_ids]
            #     & special_pair
            #     & (~self.env._special_pair_over_top_satisfied[valid_last_ids])
            # )
    ...
    def reset_idx(self, env_ids: Optional[torch.Tensor]):
        ...
        if self.cfg.is_train:
            ...
            #gate0_mask = torch.rand(n_reset, device=self.device) < .2
            ### power loop atttempt via domain randomization
            special_gate_idx = 1
            special_gate_mask = torch.rand(n_reset, device=self.device) < 0.2
            ###
            ...
            waypoint_indices[special_gate_mask] = special_gate_idx
            ...
            non_mask = ~special_gate_mask
            special_ids = torch.where(special_gate_mask)[0]
            non_ids = torch.where(non_mask)[0]

            if len(special_ids) > 0:
                # Bias many resets toward the powerloop pair by starting a bit above and
                # near the selected gate, while still facing into it.
                x_local[special_ids] = torch.empty(len(special_ids), device=self.device).uniform_(-1.2, 0.2)
                y_local[special_ids] = torch.empty(len(special_ids), device=self.device).uniform_(-0.35, 0.35)
                z_local[special_ids] = torch.empty(len(special_ids), device=self.device).uniform_(0.55, 1.25)

            if len(non_ids) > 0:
                x_local[non_ids] = torch.empty(len(non_ids), device=self.device).uniform_(-3.5, -0.3)
                y_local[non_ids] = torch.empty(len(non_ids), device=self.device).uniform_(-1.5, 1.5)
                z_local[non_ids] = torch.empty(len(non_ids), device=self.device).uniform_(0.05, 0.85)
        else:
        ...
        Testing window instead of center of gate.
        ...
        self.env._last_distance_to_goal[env_ids] = self._dis_to_active_gate(self.env._pose_drone_wrt_gate[env_ids])

        self.env._prev_x_drone_wrt_gate[env_ids] = 1.0
        if hasattr(self.env, "_special_pair_over_top_satisfied"):
            self.env._special_pair_over_top_satisfied[env_ids] = False
