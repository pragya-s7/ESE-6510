...
class DefaultQuadcopterStrategy:
    ...
    def get_rewards(self) -> torch.Tensor:
        ...
        # Use the active-to-next segment to validate gate traversal direction.
        ...
        crossed_any_plane = crossed_forward_plane | crossed_backward_plane
        gate_passed_mask = crossed_forward_plane & crossed_inside_gate & moving_along_course
        illegal_current_gate_cross_mask = crossed_any_plane & crossed_inside_gate & (~gate_passed_mask)

        cheated_mask = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        cheated_mask |= illegal_current_gate_cross_mask
        ...
        raw_forward_progress = torch.clamp(prev_distance_to_gate - distance_to_gate, min=0.0, max=0.2)

        # Dense progress only counts if the current step obeys the gate-order rules.
        forward_progress = raw_forward_progress * (~cheated_mask).float()
        gate_pass_reward = gate_passed_mask.float()

        # Legacy speed shaping is intentionally disabled for now. Keeping the reward
        # head makes it easy to re-enable from train_race.py later.
        segment_velocity_reward = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        ...
        if self.cfg.is_train:
            ...
            rewards = {
                ...
                "cheat": cheated_mask.float() * self.env.rew['cheat_reward_scale'],
    ...
    def reset_idx(self, env_ids: Optional[torch.Tensor]):
        ...
        if self.cfg.is_train:
            ...
            priority_gate_reset_prob = 0.25
            priority_gate_indices = torch.tensor([0,2,5], device=self.device, dtype=self.env._idx_wp.dtype)
            ...
            if len(priority_env_ids) > 0:
                ...
                spawn_x_local[priority_env_ids] = torch.empty(len(priority_env_ids), device=self.device).uniform_(-3.5, -0.3)
                spawn_y_local[priority_env_ids] = torch.empty(len(priority_env_ids), device=self.device).uniform_(-0.35, 0.35)
