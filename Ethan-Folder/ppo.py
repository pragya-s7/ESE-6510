...
class PPO:
    ...
    def update(self):
        ...
        for (
            ...
            if self.desired_kl is not None and self.schedule == "adaptive":
                with torch.inference_mode():
                    kl = torch.sum(
                        ...
                        + (torch.square(prev_action_stds) + torch.square(prev_mean_actions - mu))
            ...
            ratio = torch.exp(actions_log_prob - torch.squeeze(prev_log_probs, dim=-1))
            surrogate = -torch.squeeze(advantage_estimates, dim=-1) * ratio
            surrogate_clipped = -torch.squeeze(advantage_estimates, dim=-1) * torch.clamp(
